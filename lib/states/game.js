...
module.exports = {
  ...
  create: function () {
    ...
    if (prev) {
      ...
      this.player = global.player = makePlayer(
        this,
  ...
  updateEnemies: function () {
    if (this.player && !damageImmune.isActive(this.player)) {
      this.game.physics.arcade.overlap(this.enemies, this.player, this.harmPlayer, null, this)
    }
  },
  harmEnemy: function (_, enemy) {
    harm.start(this, enemy, this.playerDirection)
    this.attack.harmedEnemy = true
    enemy.damage(config.ATTACK_STRENGTH)
  },
  harmPlayer: function (player, enemy) {
    const kickbackAngle = enemy.body.center.angle(this.player.body.center)
    harm.start(this, this.player, kickbackAngle)
    damageImmune.start(this, this.player)
